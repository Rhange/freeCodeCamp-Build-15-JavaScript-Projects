...
const displayMenuBtns = (menuItems) => {
  // get unique category array
  const categories = menuItems.reduce(
    (values, item) => {
      !values.includes(item.category) ? values.push(item.category) : false;
      return values;
    },
    ["all"]
  );
  ...
  */
  ...
  // create html with each category
  ...
  // implement filter functions to filter buttons
